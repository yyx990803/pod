...
api.startApp = function (appname, callback) {
    ...
    fs.exists(app.script, function (exists) {
        ...
        Satan.executeRemote('findByScript', {script: app.script}, function (err, process) {
            if (err) return callback(err)
            if (process) return callback(null, appname.yellow + ' already running.')
            var appConf = resolvePaths({ name: app.name, script: app.script })
...
api.stopApp = function (appname, callback) {
    ...
    Satan.executeRemote('findByScript', {script: app.script}, function (err, process) {
        if (err) return callback(err)
        if (!process) return callback(null, appname.yellow + ' is not running')
        Satan.executeRemote('stopId', { id: process.pm_id }, function (err) {
            if (err) return callback(err)
