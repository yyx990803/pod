...
var api = {
    ...
    createApp: function (appname, callback) {
        if (globalConfig.apps[appname]) {
            (callback || warn)('an app with that name already exists.')
        } else {
    ...
    start: function (appname, list) {
        ...
        function run () {
            if (isRunning(list, app.script)) {
                ...
                // start daemon
                forever.startDaemon(app.script, daemonOptions)
                log(appname.yellow + ' running on port ' + app.port)
            }
        }
    },
...
function warn (msg) {
    console.warn('pod '.green + 'ERROR '.red + msg)
}
