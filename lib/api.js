...
if (!fs.existsSync(globalConfig.root)) fs.mkdirSync(globalConfig.root)
...
api.createApp = function (appname, options, callback) {
    ...
    if (globalConfig.apps[appname]) {
        return abort('EXISTS', callback, { appname: appname })
    }
...
api.removeApp = function (appname, callback) {
    ...
    if (!app) {
        return abort('NOT_FOUND', callback, { appname: appname })
    }
...
api.startApp = function (appname, callback) {
    ...
    fs.exists(app.script, function (exists) {
        if (!exists) {
            return abort('NO_SCRIPT', callback, { appname: appname, script: app.script })
        }
...
api.restartApp = function (appname, callback) {
    ...
    Satan.executeRemote('list', {}, function (err, list) {
        ...
        var runningProcs = findInList(appname, list)
        if (!runningProcs) {
            return abort('NOT_RUNNING', callback, { appname: appname })
        } else {
...
function abort (code, callback, data) {
    var msg = ERRORS[code] || 'unknown error'
