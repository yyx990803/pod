            ...
            if (!apps.length) {
                ...
                apps.forEach(function (app) {
                    var tabs = app.name.length < 6 ? '\t\t' : '\t',
                        status = app.instances ? 'ON '.green : 'OFF'.grey,
                        port = ' : ' + app.port + '\t',
                        l = app.instances && app.instances.length,
                        instances = l > 1 ? (' (' + l + ' instances)').grey : ''
                    console.log('  ' + app.name.yellow + tabs + ' - ' + status + port + instances)
                })
                console.log()
            }
            process.exit(0)
